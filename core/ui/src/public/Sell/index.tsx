...
export const Sell: React.FC<SellProps> = ({
  ...
  const handleClickNft = (nft: SingleTokenInfo) => {
    setNftSelection(nft);
  };
      ...
      <div className="candy-container">
        ...
        {!loading && nfts.length && shop ? (
          <div className="candy-container-list">
            {nfts.map((item) => (
              <div key={item.tokenMintAddress}>
                <Nft onClick={handleClickNft} nft={item} sellDetail={sellOrders[item.tokenMintAddress]} />
              </div>
            ))}
          </div>
        ) : null}
