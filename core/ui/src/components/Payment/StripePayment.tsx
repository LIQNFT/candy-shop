...
interface StripePaymentProps {
  ...
  paymentPrice: number;
  onProcessingPay: (type: BuyModalType, paymentPrice?: number, error?: PaymentError) => void;
}
  ...
  const onClickedPayCallback = (params: ConfirmStripePaymentParams) => {
    ...
    CandyShopPay.confirmPayment(params)
      .then((res: SingleBase<PaymentIntentInfo>) => {
        ...
        if (res.success && res.result) {
          onProcessingPay(BuyModalType.CONFIRMED, 11);
          console.log(`${Logger}: confirmPayment success=`, res.result);
        } else if (res.msg) {
          console.log(`${Logger}: confirmPayment failed, reason=`, res.msg);
          onProcessingPay(BuyModalType.CONFIRMED, 11, (res as any).errorDetails);
          notification(res.msg, NotificationType.Error, 5);
        }
      })
      .catch((err: any) => {
        onProcessingPay(BuyModalType.CONFIRMED, 13, err as PaymentError);
