...
import { ShopFilter as ShopFilterType } from 'model';

import '../../style/order-filter.less';

interface ShopFilterProps {
  onChange: (item: CandyShopResponse | ShopFilterType | undefined, type: 'auto' | 'manual') => any;
  selected?: CandyShopResponse;
  candyShop: CandyShop;
  filters?: ShopFilterType[] | boolean;
  selectedManual?: ShopFilterType;
      ...
      fetchAllShop(queryDto)
        .then((res: ListBase<CandyShopResponse>) => {
          ...
          setOptions((list) => {
            ...
            const duplicateList = [...list, ...result];
            const newList: CandyShopResponse[] = [];
            const memo: any = {};

            duplicateList.forEach((item) => {
              if (memo[item.candyShopAddress]) return;
              newList.push(item);
              memo[item.candyShopAddress] = true;
            });
            return newList;
          });
        })
    ...
    <div className="candy-collection-filter">
      ...
      <ul className="candy-filter-options">
        ...
        <button
          ...
          className={`candy-filter-load ${disableLoadMore ? 'candy-filter-load-disable' : undefined}`}
