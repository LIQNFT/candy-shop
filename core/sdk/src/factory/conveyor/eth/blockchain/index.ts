...
export default class BlockchainService implements BlockchainServiceInterface {
  ...
  private getProvider = async (basePublicParams: BlockchainPublicParams): Promise<{ provider: BlockchainProvider }> => {
  ...
  signCreateShopAction = async (baseParams: BlockchainPrivateParams, shop: CreateShopInterface): Promise<string> => {
  ...
  signUpdateShopAction = async (baseParams: BlockchainPrivateParams, shop: UpdateShopInterface): Promise<string> => {
    const { domain, types, value } = this.getUpdateShopSignData(shop);
    const { signer } = await this.getProviderAndSigner(baseParams);
    return await signer._signTypedData(domain, types, value);
  };

  signDeleteShopAction = async (baseParams: BlockchainPrivateParams, shop: DeleteShopInterface): Promise<string> => {
  ...
  cancelOrders = async (baseParams: BlockchainPrivateParams, orders: BlockchainOrderInterface[]) => {
    ...
    const { transactionHash } = await this.makeCancelOrderTransaction(seaport, transactionData, accountAddress, signer);
    return { transactionHash };
  };

  private getFinalCancelOrderTransactionData = async (orders: BlockchainOrderInterface[], seaport: Seaport) => {
  ...
  makeOrderConsumptionAllowance = async (
    ...
    const contract = this.getPaymentContract(order.selectedConsumption.assetInstance, provider).connect(signer);
  ...
  fulfillOrder = async (
    ...
    const { provider, signer } = await this.getProviderAndSigner(baseParams);
    const seaport = this.seaportHelper.getSeaport(provider);
    const { transactionHash } = await this.makeFulfillOrderTransaction(seaport, transactionData, signer);
    return { transactionHash };
  };
