...
import {
  ...
  BlockchainOrderPaymentAssetSplit,
  TypedDataDomain,
...
export class SeaportHelper implements SeaportHelperInterface {
  ...
  private calculatePaymentSplitAmount = (
    ...
    const splitAmount = totalAmount
      .mul(ethers.utils.parseEther(paymentSplit.percentage))
      .div(this.percentageBase)
  ...
  private getConsiderationFromOrder = (order: BlockchainOrderInterface): ConsiderationItem[] => {
    ...
    return order.payments.map((paymentSplit) => {
      const startAmount = this.calculatePaymentSplitAmount(
        paymentSplit,
        selectedConsumptionAssetInstance,
      ).toString()
